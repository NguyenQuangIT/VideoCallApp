...
import opensourceproject.videocallapp.network.ApiClient;
import opensourceproject.videocallapp.network.ApiService;
...
public class OutgoingInvitationActivity extends AppCompatActivity {
            ...
            public void onResponse(@NonNull Call<String> call, @NonNull Response<String> response) {
                if(response.isSuccessful()){
                    if(type.equals((Constants.REMOTE_MSG_INVITATION))){
                        Toast.makeText(OutgoingInvitationActivity.this, "Lời mời đã được gửi thành công!", Toast.LENGTH_SHORT).show();
                    }else if(type.equals(Constants.REMOTE_MGS_INVITATION_RESPONSE)){
                        Toast.makeText(OutgoingInvitationActivity.this, "Lời mời đã bị hủy bỏ!", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }else{
    ...
    private void cancelInvitation(String receiverToken, ArrayList<User> receivers){
        try{
            ...
            data.put(Constants.REMOTE_MSG_TYPE, Constants.REMOTE_MGS_INVITATION_RESPONSE);
            data.put(Constants.REMOTE_MGS_INVITATION_RESPONSE, Constants.REMOTE_MSG_INVITATION_CANCELLED);

            body.put(Constants.REMOTE_MSG_DATA, data);
            body.put(Constants.REMOTE_MSG_REGISTRATION_IDS, tokens);

            sendRemoteMessage(body.toString(), Constants.REMOTE_MGS_INVITATION_RESPONSE);
    ...
    private BroadcastReceiver invitationResponseReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            String type = intent.getStringExtra(Constants.REMOTE_MGS_INVITATION_RESPONSE);
            if(type!=null){
                if(type.equals(Constants.REMOTE_MSG_INVITATION_ACCEPTED)){
                    ...
                    if(rejectionCount == totalReceivers){
                        Toast.makeText(context, "Lời mời đã bị từ chối!", Toast.LENGTH_SHORT).show();
                        finish();
                    }
                }
            }
    ...
    protected void onStart() {
        ...
        LocalBroadcastManager.getInstance(getApplicationContext()).registerReceiver(
                ...
                new IntentFilter(Constants.REMOTE_MGS_INVITATION_RESPONSE)
        );
    }
